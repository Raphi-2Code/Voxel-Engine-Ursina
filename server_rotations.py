...
# Muss zur Runtime-Datei passen: Damit Gras-/Stein-Rotationen beim Export
# schon im chunks.txt stehen und beim Spielstart sofort sichtbar sind.
layer_seed = ord("y") + ord("o")
...
def infer_natural_block_type(x, y, z, heights, ho_x, ho_z):
    """Gleiche Gras/Erde/Stein-Schichtung wie in der Runtime-Datei."""
    hx = int(x - ho_x)
    hz = int(z - ho_z)
    if hx < 0 or hx >= heights.shape[0] or hz < 0 or hz >= heights.shape[1]:
        return "grass"

    surface_y = int(heights[hx, hz])
    depth = int(round(surface_y - y))

    if depth <= 0:
        return "grass"

    rand_val = abs(math.sin(x * 12.9898 + z * 78.233 + layer_seed) * 43758.5453)
...
def seeded_natural_block_rotation(x, y, z, block_type):
    """Seed-abhängige Start-Yaw-Rotation für Gras und Stein."""
    if block_type not in NATURAL_SEEDED_ROTATION_BLOCK_TYPES:
    ...
    return HORIZONTAL_ROTATION_FACES[idx]


def cube_base_from_face_position(px, py, pz, face_idx):
    off = cube_faces[int(face_idx)]
    return (
        round(float(px) - float(off[0]), 4),
        round(float(py) - float(off[1]), 4),
        round(float(pz) - float(off[2]), 4),
    )
    ...
    for xc in range(8):
        for zc in range(8):
            ...
            for i in range(len(rpx)):
                face_idx = int(rfi[i])
                bx, by, bz = cube_base_from_face_position(rpx[i], rpy[i], rpz[i], face_idx)
                block_type = infer_natural_block_type(bx, by, bz, heights, ho_x, ho_z)

                faces2.append(Vec3(rpx[i], rpy[i], rpz[i]))
                face_types.append(block_type)
                face_rotations.append(seeded_natural_block_rotation(bx, by, bz, block_type))

            if faces2 and len(faces2) == len(faces3):
                # Format: [Face-Positionen, Face-Indizes, Blocktypen, Blockrotationen]
                yo.write(str([faces2, faces3, face_types, face_rotations]) + ",")

print("Fertig! Chunks inklusive Höhlen und seed-abhängigen Start-Rotationen exportiert.")