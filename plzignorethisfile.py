...
def build():
    ...
    # Check if the player is building near the chunk borders
    if c.x % chunk_size == 0 or c.z % chunk_size == 0 or c.x % chunk_size == chunk_size - 1 or c.z % chunk_size == chunk_size - 1:
        if (c.x-17)%chunk_size==0:
            oldc=c
            c=Vec3(c.x+1,c.y,c.z)
            aqc2 = []
            aqc3 = []
            pll = 0
            pos = Vec3(c) + (0, -1.5, 0)
            ...
            for anti_f_pos in aqc2:
                if not anti_f_pos in f_pos:
                    face = Entity(model="plane", position=anti_f_pos, rotation=aqc3[aqc2.index(anti_f_pos)],
                                  parent=terrain2)
        ...
        if (c.z-17)%chunk_size==0:
            oldc = c
            c = Vec3(c.x, c.y, c.z+1)
            aqc2 = []
            aqc3 = []
            pll = 0
            pos = Vec3(c) + (0, -1.5, 0)
...
def mine():
    ...
    for element in chunk_faces2:
        ...
        rot_i = Vec3(elem[3], elem[4], elem[5])
        face = Entity(model="plane", position=pos_i, rotation=rot_i, parent=terrain2)
        # if chunk_faces3[pll]!=2 and chunk_faces3[pll]!=3:1
        new_chunk_faces2.append((pos_i[0], pos_i[1], pos_i[2]))
        new_chunk_faces.append([pos_i[0], pos_i[2]])
        new_chunk_faces3.append(chunk_faces3[pll])
        pll += 1
